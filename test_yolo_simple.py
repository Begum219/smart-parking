...
class CameraDetector:
    ...
    def process_frame(self, frame, confidence_threshold=0.05):
        ...
        for result in results:
            for box in result.boxes:
                ...
                # SADECE PARK YERİNDE OLAN NESNELERİ AL
                space_id, space_name = self.find_parking_space([x1, y1, x2, y2])
                
                # Park yerinde DEĞİLSE atla
                if not space_id:
                    continue
                
                # Park yerindeyse ARAÇ olarak kabul et!
    ...
    def run(self):
        ...
        while self.running:
            ...
            vehicles, frame = self.process_frame(frame, confidence_threshold=0.01)  # 0.05 → 0.01
            ...
            # Konsol çıktısı (HER 10 FRAME'DE BİR - daha sık!)
            if frame_count % 10 == 0 and len(vehicles) > 0:
...
if __name__ == "__main__":
    ...
    # 🔍 DEBUG MODU - True yapınca detaylı log gösterir
    DEBUG = True  # False → True (geçici olarak)
