#!/usr/bin/env python3
"""
SMART PARKING MANAGEMENT SYSTEM - FLASK STREAM VERSION
========================================================
✅ Çift kamera desteği (multi-threading)
✅ Park yeri tespiti (koordinat bazlı)
✅ Yol/yasak alan tespiti + otomatik ceza kesme (60s)
✅ Yamuk park tespiti + otomatik ceza kesme (60s)
✅ GLOBAL PENALTY SİSTEMİ - Aynı plakaya 1 KEZ ceza!
✅ REZERVASYON PATTERN - Race condition çözümü
✅ ROLLBACK MEKANİZMASI - API hatası durumunda
✅ QR kod okuma (pyzbar)
✅ OCR plaka okuma (Tesseract - opsiyonel)
✅ Araç tracking (ID bazlı)
✅ API entegrasyonu (park + ceza)
✅ Gerçek zamanlı görselleştirme
✅✅ FLASK VIDEO STREAM - Web'de kamera görüntüsü!

KULLANIM:
    pip install flask flask-cors --break-system-packages
    python multi_space.py

AYARLAR:
- Confidence: 0.01 (çok düşük - oyuncak araçlar için)
- Çözünürlük: 1920x1080 (koordinat dosyalarıyla uyumlu)
- Penalty Cooldown: 300 saniye (5 dakika)
- Flask Port: 5000 (video stream)
"""

import cv2
import json
import numpy as np
import requests
import time
from datetime import datetime
from ultralytics import YOLO
from pyzbar.pyzbar import decode
import pytesseract
import threading
from flask import Flask, Response, jsonify
from flask_cors import CORS

# ============================================================
# GLOBAL PENALTY SİSTEMİ - TÜM İHLALLER İÇİN TEK DİCTİONARY!
# ============================================================
# Plaka/QR bazında ceza takibi (hem yol hem yamuk park)
GLOBAL_PENALTY_ISSUED = {}  # {plaka: timestamp}
PENALTY_LOCK = threading.Lock()  # Thread-safe erişim
PENALTY_COOLDOWN = 300  # 5 dakika (300 saniye)
# Bir plakaya bir kez ceza kesilir (yol veya yamuk park)
# 5 dakika içinde başka ceza kesilmez!
# ============================================================

# ============================================================
# FLASK APP
# ============================================================
app = Flask(__name__)
CORS(app)  # React'ten erişim için

# ============================================================
# GLOBAL FRAME STORAGE (Flask için)
# ============================================================
CAMERA_FRAMES = {}  # {camera_id: latest_frame}
FRAME_LOCK = threading.Lock()

class CameraDetector:
    def __init__(self, camera_id, camera_name, coords_path, api_url, tesseract_path=None, debug=False):
        """
        Kamera detektörü başlatma
        
        Args:
            camera_id: Kamera ID (0, 1, 2...)
            camera_name: Kamera ismi (görsel log için)
            coords_path: Koordinat JSON dosyası yolu
            api_url: Backend API URL'i
            tesseract_path: Tesseract OCR path (opsiyonel)
            debug: Debug modu (detaylı log)
        """
        self.camera_id = camera_id
        self.camera_name = camera_name
        self.api_url = api_url
        self.debug = debug
        
        # Tesseract OCR ayarları
        if tesseract_path:
            pytesseract.pytesseract.tesseract_cmd = tesseract_path
            try:
                version = pytesseract.get_tesseract_version()
                print(f"   ✅ Tesseract {version} bulundu")
            except:
                print(f"   ⚠️  Tesseract bulunamadı: {tesseract_path}")
                print(f"       OCR çalışmayacak, sadece QR kod kullanılacak")
        
        # Koordinatları yükle
        with open(coords_path, 'r', encoding='utf-8') as f:
            self.camera_data = json.load(f)
        
        self.parking_spaces = self.camera_data['spaces']
        self.road_zones = self.camera_data.get('road_zones', [])
        
        print(f"✅ Kamera {camera_id}: {len(self.parking_spaces)} park yeri yüklendi")
        if self.road_zones:
            print(f"   🚫 {len(self.road_zones)} yol alanı yüklendi")
        
        # DEBUG - JSON formatını kontrol et
        if self.debug and self.parking_spaces:
            first_space = self.parking_spaces[0]
            print(f"   [DEBUG] İlk park yeri keys: {first_space.keys()}")
            if 'space_id' not in first_space:
                print(f"   ⚠️  UYARI: 'space_id' yok!")
        
        # YOLO model
        self.model = YOLO('yolov8n.pt')
        
        # Araç tracking
        self.tracked_vehicles = {}
        self.next_vehicle_id = 1
        self.max_distance = 100  # Piksel cinsinden maksimum tracking mesafesi
        
        # API rate limiting
        self.last_api_send = {}
        self.api_send_interval = 3  # Saniye
        
        # Yol ihlali timer sistemi
        self.road_violation_timers = {}  # {vehicle_id: {'start_time': timestamp, 'zone_id': zone}}
        self.penalty_issued = {}  # {vehicle_id: True/False} - LOKAL (kamera içi)
        self.road_violation_threshold = 60  # 60 saniye
        
        # Yamuk park timer sistemi
        self.multi_space_violation_timers = {}  # {vehicle_id: {'start_time': timestamp, 'spaces': [...]}}
        self.multi_space_penalty_issued = {}  # {vehicle_id: True/False} - LOKAL (kamera içi)
        self.multi_space_threshold = 60  # 60 saniye
        
        # Thread kontrol
        self.running = True
    
    # ============================================================
    # GEOMETRİ FONKSİYONLARI
    # ============================================================
    
    def point_in_polygon(self, point, polygon):
        """
        Ray casting algoritması ile nokta-polygon içinde mi kontrolü
        
        Args:
            point: (x, y) tuple
            polygon: [(x1, y1), (x2, y2), ...] koordinat listesi
            
        Returns:
            bool: Nokta polygon içindeyse True
        """
        x, y = point
        n = len(polygon)
        inside = False
        
        p1x, p1y = polygon[0]
        for i in range(1, n + 1):
            p2x, p2y = polygon[i % n]
            if y > min(p1y, p2y):
                if y <= max(p1y, p2y):
                    if x <= max(p1x, p2x):
                        if p1y != p2y:
                            xinters = (y - p1y) * (p2x - p1x) / (p2y - p1y) + p1x
                        if p1x == p2x or x <= xinters:
                            inside = not inside
            p1x, p1y = p2x, p2y
        
        return inside
    
    def get_centroid(self, bbox):
        """
        Bounding box merkezi
        
        Args:
            bbox: [x1, y1, x2, y2]
            
        Returns:
            tuple: (cx, cy) merkez koordinatı
        """
        x1, y1, x2, y2 = bbox
        return ((x1 + x2) // 2, (y1 + y2) // 2)
    
    def distance(self, p1, p2):
        """
        İki nokta arası Öklid mesafesi
        
        Args:
            p1: (x1, y1)
            p2: (x2, y2)
            
        Returns:
            float: Mesafe
        """
        return np.sqrt((p1[0] - p2[0])**2 + (p1[1] - p2[1])**2)
    
    # ============================================================
    # PARK YERİ VE YOL TESPİTİ
    # ============================================================
    
    def find_parking_space(self, bbox):
        """
        Araç hangi park yerinde?
        
        Args:
            bbox: [x1, y1, x2, y2]
            
        Returns:
            tuple: (space_id, space_name) veya (None, None)
        """
        center = self.get_centroid(bbox)
        
        if self.debug:
            print(f"  [DEBUG] Araç merkezi: {center}")
        
        for space in self.parking_spaces:
            if 'space_id' not in space:
                print(f"  [HATA] Park yerinde 'space_id' yok! Keys: {space.keys()}")
                continue
            
            if self.point_in_polygon(center, space['coordinates']):
                if self.debug:
                    print(f"  [DEBUG] ✅ Park yeri bulundu: {space['space_id']}")
                return space['space_id'], space['space_id']
        
        if self.debug:
            print(f"  [DEBUG] ❌ Park yeri bulunamadı!")
        
        return None, None
    
    def is_in_road_zone(self, bbox):
        """
        Araç yol alanında mı? (YASAK!)
        
        Args:
            bbox: [x1, y1, x2, y2]
            
        Returns:
            tuple: (bool, zone_id)
        """
        center = self.get_centroid(bbox)
        
        if self.debug:
            print(f"  [DEBUG YOL] Kontrol edilen merkez: {center}")
        
        for zone in self.road_zones:
            if self.point_in_polygon(center, zone['coordinates']):
                if self.debug:
                    print(f"  [DEBUG YOL] ✅ Yol alanında! Zone: {zone['zone_id']}")
                return True, zone['zone_id']
        
        if self.debug:
            print(f"  [DEBUG YOL] ❌ Yol alanında değil")
        
        return False, None
    
    def check_multi_space_violation(self, bbox):
        """
        Birden fazla park yeri işgali kontrolü (Yamuk Park)
        
        Args:
            bbox: [x1, y1, x2, y2]
            
        Returns:
            tuple: (bool, [space_ids]) - Yamuk park varsa True ve işgal edilen park yerleri listesi
        """
        x1, y1, x2, y2 = bbox
        
        # Bbox'ın 4 köşesini kontrol et
        corners = [
            (x1, y1),      # Sol üst
            (x2, y1),      # Sağ üst
            (x1, y2),      # Sol alt
            (x2, y2)       # Sağ alt
        ]
        
        occupied_spaces = set()
        
        for corner in corners:
            for space in self.parking_spaces:
                if 'space_id' not in space:
                    continue
                if self.point_in_polygon(corner, space['coordinates']):
                    occupied_spaces.add(space['space_id'])
        
        # Eğer 2 veya daha fazla park yeri işgal ediliyorsa
        if len(occupied_spaces) >= 2:
            return True, sorted(list(occupied_spaces))
        
        return False, []
    
    # ============================================================
    # YOL İHLALİ TIMER FONKSİYONLARI
    # ============================================================
    
    def start_road_violation_timer(self, vehicle_id, zone_id):
        """Yol ihlali timer'ını başlat"""
        if vehicle_id not in self.road_violation_timers:
            self.road_violation_timers[vehicle_id] = {
                'start_time': time.time(),
                'zone_id': zone_id
            }
            self.penalty_issued[vehicle_id] = False
            print(f"🚗 [{self.camera_name}] Araç {vehicle_id} yola girdi - Timer başlatıldı! (60s)")
    
    def check_road_violation_timer(self, vehicle_id):
        """Timer kontrolü - 60s geçti mi? Ceza kesildi mi?"""
        if vehicle_id in self.road_violation_timers:
            elapsed = time.time() - self.road_violation_timers[vehicle_id]['start_time']
            
            # Ceza daha önce kesildi mi kontrol et
            if self.penalty_issued.get(vehicle_id, False):
                return False, 0
            
            if elapsed >= self.road_violation_threshold:
                print(f"⏰ [{self.camera_name}] Araç {vehicle_id} yolda {int(elapsed)} saniye!")
                print(f"   ⚠️  1 dakika eşiği aşıldı → CEZA KESİLECEK!")
                return True, elapsed
            else:
                remaining = self.road_violation_threshold - int(elapsed)
                if int(elapsed) % 10 == 0:  # Her 10 saniyede log
                    print(f"⏳ [{self.camera_name}] Araç {vehicle_id} yolda {int(elapsed)}s (Kalan: {remaining}s)")
                return False, elapsed
        return False, 0
    
    def clear_road_violation_timer(self, vehicle_id):
        """Araç yol alanından çıktı - timer'ı temizle"""
        if vehicle_id in self.road_violation_timers:
            elapsed = time.time() - self.road_violation_timers[vehicle_id]['start_time']
            print(f"✅ [{self.camera_name}] Araç {vehicle_id} yoldan çıktı ({int(elapsed)}s sonra)")
            del self.road_violation_timers[vehicle_id]
            if vehicle_id in self.penalty_issued:
                del self.penalty_issued[vehicle_id]
    
    def get_road_timer_display(self, vehicle_id):
        """Ekranda gösterilecek timer bilgisi"""
        if vehicle_id in self.road_violation_timers:
            elapsed = int(time.time() - self.road_violation_timers[vehicle_id]['start_time'])
            remaining = max(0, self.road_violation_threshold - elapsed)
            
            if self.penalty_issued.get(vehicle_id, False):
                return "CEZA KESİLDİ!", (0, 0, 255)  # Kırmızı
            else:
                return f"Timer: {remaining}s", (0, 165, 255)  # Turuncu
        return None, None
    
    # ============================================================
    # YAMUK PARK TIMER FONKSİYONLARI
    # ============================================================
    
    def start_multi_space_violation_timer(self, vehicle_id, spaces):
        """Yamuk park timer'ını başlat"""
        if vehicle_id not in self.multi_space_violation_timers:
            self.multi_space_violation_timers[vehicle_id] = {
                'start_time': time.time(),
                'spaces': spaces
            }
            self.multi_space_penalty_issued[vehicle_id] = False
            print(f"🚗 [{self.camera_name}] Araç {vehicle_id} YAMUK PARK! İşgal: {', '.join(spaces)} - Timer başlatıldı! (60s)")
    
    def check_multi_space_violation_timer(self, vehicle_id):
        """Yamuk park timer kontrolü"""
        if vehicle_id in self.multi_space_violation_timers:
            elapsed = time.time() - self.multi_space_violation_timers[vehicle_id]['start_time']
            
            if self.multi_space_penalty_issued.get(vehicle_id, False):
                return False, 0
            
            if elapsed >= self.multi_space_threshold:
                print(f"⏰ [{self.camera_name}] Araç {vehicle_id} {int(elapsed)}s yamuk park yaptı!")
                print(f"   ⚠️  1 dakika eşiği aşıldı → CEZA KESİLECEK!")
                return True, elapsed
            else:
                remaining = self.multi_space_threshold - int(elapsed)
                if int(elapsed) % 10 == 0:
                    print(f"⏳ [{self.camera_name}] Araç {vehicle_id} yamuk park {int(elapsed)}s (Kalan: {remaining}s)")
                return False, elapsed
        return False, 0
    
    def clear_multi_space_violation_timer(self, vehicle_id):
        """Yamuk park timer temizle"""
        if vehicle_id in self.multi_space_violation_timers:
            elapsed = time.time() - self.multi_space_violation_timers[vehicle_id]['start_time']
            print(f"✅ [{self.camera_name}] Araç {vehicle_id} düzeltildi ({int(elapsed)}s sonra)")
            del self.multi_space_violation_timers[vehicle_id]
            if vehicle_id in self.multi_space_penalty_issued:
                del self.multi_space_penalty_issued[vehicle_id]
    
    def get_multi_space_timer_display(self, vehicle_id):
        """Yamuk park ekran gösterimi"""
        if vehicle_id in self.multi_space_violation_timers:
            elapsed = int(time.time() - self.multi_space_violation_timers[vehicle_id]['start_time'])
            remaining = max(0, self.multi_space_threshold - elapsed)
            
            if self.multi_space_penalty_issued.get(vehicle_id, False):
                return "YAMUK PARK CEZA!", (255, 0, 255)  # Mor
            else:
                return f"Yamuk: {remaining}s", (255, 165, 0)  # Turuncu
        return None, None
    
    # ============================================================
    # ARAÇ TRACKING
    # ============================================================
    
    def track_vehicle(self, bbox, space_id, space_name, plate, conf):
        """
        Araç tracking - ID tabanlı
        
        Args:
            bbox: [x1, y1, x2, y2]
            space_id: Park yeri ID
            space_name: Park yeri ismi
            plate: Plaka/QR
            conf: YOLO confidence
            
        Returns:
            tuple: (vehicle_id, vehicle_data)
        """
        centroid = self.get_centroid(bbox)
        
        # En yakın mevcut aracı bul
        closest_id = None
        closest_dist = self.max_distance
        
        for vid, vdata in list(self.tracked_vehicles.items()):
            dist = self.distance(centroid, vdata['centroid'])
            if dist < closest_dist:
                closest_dist = dist
                closest_id = vid
        
        if closest_id is not None:
            # Mevcut aracı güncelle
            self.tracked_vehicles[closest_id].update({
                'centroid': centroid,
                'space_id': space_id,
                'space_name': space_name,
                'plate': plate,
                'confidence': conf,
                'frames': self.tracked_vehicles[closest_id]['frames'] + 1
            })
            return closest_id, self.tracked_vehicles[closest_id]
        else:
            # Yeni araç
            vid = self.next_vehicle_id
            self.next_vehicle_id += 1
            self.tracked_vehicles[vid] = {
                'centroid': centroid,
                'space_id': space_id,
                'space_name': space_name,
                'plate': plate,
                'confidence': conf,
                'frames': 1
            }
            return vid, self.tracked_vehicles[vid]
    
    # ============================================================
    # QR KOD VE PLAKA OKUMA
    # ============================================================
    
    def read_qr_code(self, frame, bbox):
        """
        QR kod oku - TÜM FRAME'DEN!
        
        Args:
            frame: Kamera frame'i
            bbox: Araç bounding box [x1, y1, x2, y2]
            
        Returns:
            str: QR kod verisi veya boş string
        """
        try:
            # Tüm frame'den QR oku
            qr_codes = decode(frame)
            
            if qr_codes:
                # QR bulundu - araç bbox'ına yakın mı kontrol et
                x1, y1, x2, y2 = bbox
                
                for qr in qr_codes:
                    # QR kodun merkezi
                    qr_rect = qr.rect
                    qr_center = (qr_rect.left + qr_rect.width // 2,
                                qr_rect.top + qr_rect.height // 2)
                    
                    # QR kod araç bbox içinde mi veya yakınında mı?
                    margin = 100
                    if (x1 - margin <= qr_center[0] <= x2 + margin and
                        y1 - margin <= qr_center[1] <= y2 + margin):
                        
                        qr_data = qr.data.decode('utf-8')
                        if self.debug:
                            print(f"  [QR OKUNDU!] {qr_data} @ {qr_center}")
                        return qr_data
        
        except:
            # QR okuma hatası - sessizce atla
            pass
        
        return ""
    
    def extract_plate_region(self, frame, bbox):
        """
        Plaka bölgesi (alt %30)
        
        Args:
            frame: Kamera frame'i
            bbox: [x1, y1, x2, y2]
            
        Returns:
            numpy.ndarray: Plaka bölgesi
        """
        x1, y1, x2, y2 = bbox
        height = y2 - y1
        plate_y1 = int(y2 - height * 0.3)
        return frame[plate_y1:y2, x1:x2]
    
    def read_plate(self, plate_region):
        """
        OCR ile plaka okuma - İYİLEŞTİRİLMİŞ
        
        Args:
            plate_region: Plaka bölgesi frame
            
        Returns:
            str: Plaka metni veya boş string
        """
        if plate_region.size == 0:
            return ""
        
        # Tesseract yoksa sessizce geri dön
        try:
            import pytesseract
            if not hasattr(pytesseract.pytesseract, 'tesseract_cmd'):
                return ""
        except:
            return ""
        
        try:
            # Resize - küçükse büyüt
            h, w = plate_region.shape[:2]
            if h < 50 or w < 100:
                scale = max(2, 100 / w)
                plate_region = cv2.resize(plate_region, None, fx=scale, fy=scale,
                                         interpolation=cv2.INTER_CUBIC)
            
            # Gri tonlama
            gray = cv2.cvtColor(plate_region, cv2.COLOR_BGR2GRAY)
            
            # Gürültü azaltma
            denoised = cv2.fastNlMeansDenoising(gray)
            
            # Kontrast artırma
            clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))
            enhanced = clahe.apply(denoised)
            
            # Thresholding (hem siyah hem beyaz plakalar için)
            _, binary1 = cv2.threshold(enhanced, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            _, binary2 = cv2.threshold(enhanced, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
            
            # OCR config
            config = '--psm 7 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'
            
            # Her ikisini de dene
            text1 = pytesseract.image_to_string(binary1, config=config).strip()
            text2 = pytesseract.image_to_string(binary2, config=config).strip()
            
            # Uzun olanı al
            result = text1 if len(text1) > len(text2) else text2
            
            if self.debug and result:
                print(f"  [PLAKA OKUNDU!] {result}")
            
            if len(result) >= 5:
                return result
            return ""
        
        except:
            # Tesseract hatası - sessizce atla
            return ""
    
    # ============================================================
    # API FONKSİYONLARI
    # ============================================================
    
    def send_to_api(self, vehicle_data):
        """
        Normal park - API'ye gönder
        
        Args:
            vehicle_data: Araç bilgileri dict
        """
        vehicle_id = vehicle_data['vehicle_id']
        
        # Rate limiting
        now = time.time()
        if vehicle_id in self.last_api_send:
            if now - self.last_api_send[vehicle_id] < self.api_send_interval:
                return
        
        try:
            payload = {
                "cameraId": self.camera_id,
                "cameraName": self.camera_name,
                "vehicleId": vehicle_id,
                
                "parkingSpaceName": vehicle_data['parking_space_name'],
                "qrCode": vehicle_data.get('qr'),
                "plateNumber": vehicle_data.get('plate'),
                "confidence": float(vehicle_data['confidence']),
                "framesDetected": vehicle_data['frames'],
                "detectionTime": datetime.now().isoformat(),
                "status": "occupied"
            }
            
            response = requests.post(
                f"{self.api_url}/api/parking/detect",
                json=payload,
                timeout=5,
                headers={'Content-Type': 'application/json'}
            )
            
            if response.status_code in [200, 201]:
                print(f"[{self.camera_name}] ✅ Araç {vehicle_id} API'ye gönderildi")
                self.last_api_send[vehicle_id] = now
            else:
                print(f"[{self.camera_name}] ❌ API RED: {response.status_code} - {response.text[:300]}")
        
        except Exception as e:
            print(f"[{self.camera_name}] ❌ API EXCEPTION: {e}")

    # ✅ YENİ EKLEME: Araç ayrıldığında backend'e bildir
    def send_vehicle_left(self, vehicle_id):
        """Araç tracking'den çıktı — backend'e bildir"""
        try:
            response = requests.post(
                f"{self.api_url}/api/parking/vehicle-left/{vehicle_id}",
                timeout=5
            )
            if response.status_code == 200:
                print(f"[{self.camera_name}] ✅ Araç {vehicle_id} ayrıldı — session kapatıldı")
        except Exception:
            pass
    
    def issue_penalty(self, vehicle_data, zone_id):
        """
        🚨 YOL İHLALİ CEZASI - GLOBAL PENALTY + ROLLBACK
        
        Args:
            vehicle_data: Araç bilgileri dict
            zone_id: Yol zone ID
        """
        global GLOBAL_PENALTY_ISSUED, PENALTY_LOCK, PENALTY_COOLDOWN
        
        vehicle_id = vehicle_data['vehicle_id']
        plate_or_qr = vehicle_data.get('qr') or vehicle_data.get('plate')
        
        # Plaka/QR yoksa ceza kesme AMA lokal flag'i set et (spam önleme!)
        if not plate_or_qr:
            print(f"⚠️  [{self.camera_name}] Araç {vehicle_id} - Plaka/QR okunamadı, ceza kesilemiyor!")
            self.penalty_issued[vehicle_id] = True  # ✨ YENİ: Tekrar denemeyi engelle!
            return
        
        # GLOBAL kontrol + REZERVASYON (RACE CONDITION FIX!)
        with PENALTY_LOCK:
            current_time = time.time()
            
            # Bu plakaya daha önce ceza kesildi mi?
            if plate_or_qr in GLOBAL_PENALTY_ISSUED:
                last_penalty_time = GLOBAL_PENALTY_ISSUED[plate_or_qr]
                elapsed_since_penalty = current_time - last_penalty_time
                
                if elapsed_since_penalty < PENALTY_COOLDOWN:
                    remaining = int(PENALTY_COOLDOWN - elapsed_since_penalty)
                    print(f"⏸️  [{self.camera_name}] {plate_or_qr} - GLOBAL: Ceza zaten kesilmiş! (Cooldown: {remaining}s kaldı)")
                    # Lokal flag'i de set et
                    self.penalty_issued[vehicle_id] = True
                    return
            
            # KRİTİK: FLAG'İ ŞİMDİ SET ET (REZERVASYON)!
            # API çağrısından ÖNCE set ediyoruz ki diğer kamera görsün
            GLOBAL_PENALTY_ISSUED[plate_or_qr] = current_time
            print(f"[GLOBAL] 🔒 {plate_or_qr} - Yol ihlali ceza flag'i set edildi (300s cooldown)")
        
        # Ceza kesme mesajı
        print(f"\n{'='*60}")
        print(f"🚨 YOL İHLALİ CEZASI! 🚨")
        print(f"Kamera: {self.camera_name}")
        print(f"Araç ID: {vehicle_id}")
        print(f"Alan: {zone_id} (YOL - YASAK!)")
        print(f"Plaka/QR: {plate_or_qr}")
        print(f"Ceza: 500 TL")
        print(f"Sebep: 1 dakikadan fazla yolda park")
        print(f"{'='*60}\n")
        
        try:
            payload = {
                "cameraName": self.camera_name,
                "detectionVehicleId": str(vehicle_id),
                "zoneId": zone_id,
                "qrCode": vehicle_data.get('qr'),
                "plateNumber": vehicle_data.get('plate'),
                "amount": 500.00
            }
            
            print(f"[DEBUG] API Payload: {payload}")
            
            response = requests.post(
                f"{self.api_url}/api/Penalty/issue-road-violation",
                json=payload,
                timeout=5,
                headers={'Content-Type': 'application/json'}
            )
            
            if response.status_code in [200, 201]:
                print(f"[{self.camera_name}] ✅ API Response:")
                print(f"  Status: {response.status_code}")
                print(f"  Body: {response.text[:500]}")
                
                # Lokal flag'i set et
                self.penalty_issued[vehicle_id] = True
                
                print(f"[GLOBAL] ✅ {plate_or_qr} yol ihlali cezası başarıyla kesildi!")
            else:
                print(f"[{self.camera_name}] ❌ API HATASI!")
                print(f"  Status: {response.status_code}")
                print(f"  Body: {response.text[:500]}")
                
                # HATA DURUMUNDA FLAG'İ GERİ AL (ROLLBACK)!
                with PENALTY_LOCK:
                    if plate_or_qr in GLOBAL_PENALTY_ISSUED:
                        del GLOBAL_PENALTY_ISSUED[plate_or_qr]
                        print(f"[GLOBAL] ⚠️  {plate_or_qr} - Yol ihlali rezervasyonu iptal edildi (API hatası)")
                
                # ✨ YENİ: Lokal flag'i set et (spam önleme - başarısız olsa bile tekrar deneme!)
                self.penalty_issued[vehicle_id] = True
        
        except Exception as e:
            print(f"[{self.camera_name}] ❌ Yol ihlali API Exception: {e}")
            
            # EXCEPTION DURUMUNDA DA FLAG'İ GERİ AL (ROLLBACK)!
            with PENALTY_LOCK:
                if plate_or_qr in GLOBAL_PENALTY_ISSUED:
                    del GLOBAL_PENALTY_ISSUED[plate_or_qr]
                    print(f"[GLOBAL] ⚠️  {plate_or_qr} - Yol ihlali rezervasyonu iptal edildi (Exception)")
            
            # ✨ YENİ: Lokal flag'i set et (spam önleme - exception olsa bile tekrar deneme!)
            self.penalty_issued[vehicle_id] = True
    
    def issue_multi_space_penalty(self, vehicle_data, spaces):
        """
        🚨 YAMUK PARK CEZASI - GLOBAL PENALTY + ROLLBACK
        
        Args:
            vehicle_data: Araç bilgileri dict
            spaces: İşgal edilen park yerleri listesi
        """
        global GLOBAL_PENALTY_ISSUED, PENALTY_LOCK, PENALTY_COOLDOWN
        
        vehicle_id = vehicle_data['vehicle_id']
        plate_or_qr = vehicle_data.get('qr') or vehicle_data.get('plate')
        
        # Plaka/QR yoksa ceza kesme AMA lokal flag'i set et (spam önleme!)
        if not plate_or_qr:
            print(f"⚠️  [{self.camera_name}] Araç {vehicle_id} - Plaka/QR okunamadı, ceza kesilemiyor!")
            self.multi_space_penalty_issued[vehicle_id] = True  # ✨ YENİ: Tekrar denemeyi engelle!
            return
        
        # GLOBAL kontrol + REZERVASYON (RACE CONDITION FIX!)
        with PENALTY_LOCK:
            current_time = time.time()
            
            # Bu plakaya daha önce ceza kesildi mi?
            if plate_or_qr in GLOBAL_PENALTY_ISSUED:
                last_penalty_time = GLOBAL_PENALTY_ISSUED[plate_or_qr]
                elapsed_since_penalty = current_time - last_penalty_time
                
                if elapsed_since_penalty < PENALTY_COOLDOWN:
                    remaining = int(PENALTY_COOLDOWN - elapsed_since_penalty)
                    print(f"⏸️  [{self.camera_name}] {plate_or_qr} - GLOBAL: Ceza zaten kesilmiş! (Cooldown: {remaining}s kaldı)")
                    # Lokal flag'i de set et
                    self.multi_space_penalty_issued[vehicle_id] = True
                    return
            
            # KRİTİK: FLAG'İ ŞİMDİ SET ET (REZERVASYON)!
            GLOBAL_PENALTY_ISSUED[plate_or_qr] = current_time
            print(f"[GLOBAL] 🔒 {plate_or_qr} - Yamuk park ceza flag'i set edildi (300s cooldown)")
        
        # Ceza kesme mesajı
        print(f"\n{'='*60}")
        print(f"🚨 YAMUK PARK CEZASI! 🚨")
        print(f"Kamera: {self.camera_name}")
        print(f"Araç ID: {vehicle_id}")
        print(f"İşgal Edilen Park Yerleri: {', '.join(spaces)}")
        print(f"Plaka/QR: {plate_or_qr}")
        print(f"Ceza: 300 TL")
        print(f"Sebep: Birden fazla park yeri işgali (Yamuk Park)")
        print(f"{'='*60}\n")
        
        try:
            payload = {
                "cameraName": self.camera_name,
                "detectionVehicleId": str(vehicle_id),
                "parkingSpaceIds": spaces,
                "qrCode": vehicle_data.get('qr'),
                "plateNumber": vehicle_data.get('plate'),
                "amount": 300.00,
                "violationType": "multi_space_violation"
            }
            
            print(f"[DEBUG] API Payload: {payload}")
            
            response = requests.post(
                f"{self.api_url}/api/Penalty/issue-multi-space-violation",
                json=payload,
                timeout=5,
                headers={'Content-Type': 'application/json'}
            )
            
            if response.status_code in [200, 201]:
                print(f"[{self.camera_name}] ✅ API Response:")
                print(f"  Status: {response.status_code}")
                print(f"  Body: {response.text[:500]}")
                
                # Lokal flag'i set et
                self.multi_space_penalty_issued[vehicle_id] = True
                
                print(f"[GLOBAL] ✅ {plate_or_qr} yamuk park cezası başarıyla kesildi!")
            else:
                print(f"[{self.camera_name}] ❌ API HATASI!")
                print(f"  Status: {response.status_code}")
                print(f"  Body: {response.text[:500]}")
                
                # HATA DURUMUNDA FLAG'İ GERİ AL (ROLLBACK)!
                with PENALTY_LOCK:
                    if plate_or_qr in GLOBAL_PENALTY_ISSUED:
                        del GLOBAL_PENALTY_ISSUED[plate_or_qr]
                        print(f"[GLOBAL] ⚠️  {plate_or_qr} - Yamuk park rezervasyonu iptal edildi (API hatası)")
                
                # ✨ YENİ: Lokal flag'i set et (spam önleme - başarısız olsa bile tekrar deneme!)
                self.multi_space_penalty_issued[vehicle_id] = True
        
        except Exception as e:
            print(f"[{self.camera_name}] ❌ Yamuk park API Exception: {e}")
            
            # EXCEPTION DURUMUNDA DA FLAG'İ GERİ AL (ROLLBACK)!
            with PENALTY_LOCK:
                if plate_or_qr in GLOBAL_PENALTY_ISSUED:
                    del GLOBAL_PENALTY_ISSUED[plate_or_qr]
                    print(f"[GLOBAL] ⚠️  {plate_or_qr} - Yamuk park rezervasyonu iptal edildi (Exception)")
            
            # ✨ YENİ: Lokal flag'i set et (spam önleme - exception olsa bile tekrar deneme!)
            self.multi_space_penalty_issued[vehicle_id] = True
    
    # ============================================================
    # FRAME İŞLEME
    # ============================================================
    
    def process_frame(self, frame, confidence_threshold=0.05):
        """
        Frame işleme ana fonksiyonu
        
        Args:
            frame: Kamera frame'i
            confidence_threshold: YOLO confidence eşiği
            
        Returns:
            tuple: (detected_vehicles, annotated_frame)
        """
        # YOLO ile tespit
        results = self.model(frame, conf=confidence_threshold, verbose=False)
        detected_vehicles = []
        current_frame_vehicles = set()
        
        for result in results:
            for box in result.boxes:
                x1, y1, x2, y2 = map(int, box.xyxy[0])
                class_id = int(box.cls[0])
                confidence = float(box.conf[0])
                
                # Park yeri ve yol kontrolü
                space_id, space_name = self.find_parking_space([x1, y1, x2, y2])
                in_road, road_zone_id = self.is_in_road_zone([x1, y1, x2, y2])
                
                # Yamuk park kontrolü
                is_multi_space, occupied_spaces = self.check_multi_space_violation([x1, y1, x2, y2])
                
                # Park yerinde DE yolda DA DEĞİLSE atla (gürültü)
                if not space_id and not in_road:
                    if self.debug:
                        print(f"  [ATLA] Park yok VE yol yok → gürültü")
                    continue
                
                # QR/Plaka oku
                qr_data = self.read_qr_code(frame, [x1, y1, x2, y2])
                plate_text = qr_data if qr_data else ""
                
                if not plate_text:
                    plate_region = self.extract_plate_region(frame, [x1, y1, x2, y2])
                    plate_text = self.read_plate(plate_region)
                
                # Tracking
                vehicle_id, vehicle_data = self.track_vehicle(
                    [x1, y1, x2, y2], space_id, space_name, plate_text, confidence
                )
                
                current_frame_vehicles.add(vehicle_id)
                
                vehicle_info = {
                    'vehicle_id': vehicle_id,
                    'bbox': [x1, y1, x2, y2],
                    'parking_space_id': space_id,
                    'parking_space_name': space_name,
                    'plate': plate_text,
                    'qr': qr_data,
                    'confidence': confidence,
                    'frames': vehicle_data['frames'],
                    'in_road': in_road,
                    'road_zone_id': road_zone_id,
                    'is_multi_space': is_multi_space,
                    'occupied_spaces': occupied_spaces
                }
                detected_vehicles.append(vehicle_info)
                
                # ============================================================
                # YOL İHLALİ LOJİĞİ
                # ============================================================
                if in_road:
                    self.start_road_violation_timer(vehicle_id, road_zone_id)
                    should_penalize, elapsed = self.check_road_violation_timer(vehicle_id)
                    
                    if should_penalize:
                        self.issue_penalty(vehicle_info, road_zone_id)
                else:
                    self.clear_road_violation_timer(vehicle_id)
                
                # ============================================================
                # YAMUK PARK LOJİĞİ
                # ============================================================
                if space_id and is_multi_space:
                    self.start_multi_space_violation_timer(vehicle_id, occupied_spaces)
                    should_penalize_multi, elapsed_multi = self.check_multi_space_violation_timer(vehicle_id)
                    
                    if should_penalize_multi:
                        self.issue_multi_space_penalty(vehicle_info, occupied_spaces)
                elif not is_multi_space:
                    self.clear_multi_space_violation_timer(vehicle_id)
                
                # Normal park API
                if space_id and vehicle_data['frames'] >= 3:
                    self.send_to_api(vehicle_info)
                # ============================================================
                # ÇİZİM
                # ============================================================
                # Renk: Yol=Kırmızı, Yamuk=Mor, Normal=Yeşil
                if in_road:
                    color = (0, 0, 255)  # Kırmızı
                elif is_multi_space:
                    color = (255, 0, 255)  # Mor
                else:
                    color = (0, 255, 0)  # Yeşil
                
                thickness = 3 if (in_road or is_multi_space) else 2
                cv2.rectangle(frame, (x1, y1), (x2, y2), color, thickness)
                
                # ID
                cv2.putText(frame, f"ID:{vehicle_id}", (x1, y1 - 50),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                
                # Yol ihlali gösterimi
                if in_road:
                    cv2.putText(frame, "YASAK ALAN!", (x1, y1 - 70),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                    timer_text, timer_color = self.get_road_timer_display(vehicle_id)
                    if timer_text:
                        cv2.putText(frame, timer_text, (x1, y1 - 90),
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, timer_color, 2)
                
                # Yamuk park gösterimi
                if is_multi_space:
                    cv2.putText(frame, "YAMUK PARK!", (x1, y1 - 70),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 255), 2)
                    timer_text_multi, timer_color_multi = self.get_multi_space_timer_display(vehicle_id)
                    if timer_text_multi:
                        cv2.putText(frame, timer_text_multi, (x1, y1 - 90),
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, timer_color_multi, 2)
                    # İşgal edilen park yerleri
                    spaces_text = f"Isgal: {', '.join(occupied_spaces)}"
                    cv2.putText(frame, spaces_text, (x1, y1 - 110),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                
                # Park yeri
                if space_id:
                    cv2.putText(frame, f"Space: {space_id}", (x1, y1 - 30),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                
                # Plaka/QR
                if qr_data:
                    cv2.putText(frame, f"QR: {qr_data[:15]}", (x1, y1 - 10),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                elif plate_text:
                    cv2.putText(frame, f"Plaka: {plate_text}", (x1, y1 - 10),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
                
                # Confidence
                cv2.putText(frame, f"Conf: {confidence:.2f}", (x1, y2 + 20),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        
        # ✅ YENİ: Kaybolan araçları tespit et, backend'e bildir ve hafızadan temizle
        disappeared = set(self.tracked_vehicles.keys()) - current_frame_vehicles
        for vehicle_id in disappeared:
            self.send_vehicle_left(vehicle_id)
            del self.tracked_vehicles[vehicle_id]
            self.clear_road_violation_timer(vehicle_id)
            self.clear_multi_space_violation_timer(vehicle_id)
            if vehicle_id in self.last_api_send:
                del self.last_api_send[vehicle_id]
        
        return detected_vehicles, frame
    
    def draw_zones(self, frame):
        """Park yerleri + Yolları çiz"""
        # Park yerleri (SARI)
        for space in self.parking_spaces:
            coords = np.array(space['coordinates'], dtype=np.int32)
            cv2.polylines(frame, [coords], True, (255, 255, 0), 1)
            
            M = cv2.moments(coords)
            if M["m00"] != 0:
                cx = int(M["m10"] / M["m00"])
                cy = int(M["m01"] / M["m00"])
                
                label = space.get('space_id', space.get('id', '?'))
                
                cv2.putText(frame, str(label), (cx - 10, cy),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
        
        # Yol alanları (KIRMIZI)
        for zone in self.road_zones:
            coords = np.array(zone['coordinates'], dtype=np.int32)
            cv2.polylines(frame, [coords], True, (0, 0, 255), 2)
            
            overlay = frame.copy()
            cv2.fillPoly(overlay, [coords], (0, 0, 255))
            cv2.addWeighted(overlay, 0.2, frame, 0.8, 0, frame)
    
    def run(self):
        """Ana kamera döngüsü"""
        global CAMERA_FRAMES, FRAME_LOCK
        
        cap = cv2.VideoCapture(self.camera_id, cv2.CAP_DSHOW)
        
        if not cap.isOpened():
            print(f"❌ Kamera {self.camera_id} açılamadı!")
            self.running = False
            return
        
        # ÇÖZÜNÜRLÜK - Koordinat dosyalarıyla uyumlu!
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1920)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 1080)
        
        actual_width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
        actual_height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
        
        print(f"✅ {self.camera_name} başlatıldı ({int(actual_width)}x{int(actual_height)})")
        
        frame_count = 0
        
        while self.running:
            ret, frame = cap.read()
            if not ret:
                break
            
            frame_count += 1
            
            # Tespit - ÇOK DÜŞÜK THRESHOLD (oyuncak araçlar için!)
            vehicles, frame = self.process_frame(frame, confidence_threshold=0.03)
            
            # Zonu çiz
            self.draw_zones(frame)
            
            # Bilgi paneli
            info = f"[{self.camera_name}] Araclar: {len(vehicles)} | Takip: {len(self.tracked_vehicles)}"
            cv2.putText(frame, info, (10, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            
            # ✨ FLASK İÇİN FRAME KAYDET
            with FRAME_LOCK:
                CAMERA_FRAMES[self.camera_id] = frame.copy()
            
            # Göster
            cv2.imshow(self.camera_name, frame)
            
            # Konsol çıktısı (her 30 frame'de bir)
            if frame_count % 30 == 0 and len(vehicles) > 0:
                print(f"\n[{self.camera_name}] Araçlar: {len(vehicles)}")
                for v in vehicles:
                    if v['in_road']:
                        status = "🚫 YOLDA!"
                    elif v['is_multi_space']:
                        status = f"⚠️  YAMUK PARK! ({', '.join(v['occupied_spaces'])})"
                    else:
                        status = f"✅ {v['parking_space_name']}"
                    plate = v['qr'] or v['plate'] or "Okunamadı"
                    print(f"  Araç {v['vehicle_id']}: {status} | Plaka: {plate}")
            
            # ESC çıkış
            if cv2.waitKey(1) & 0xFF == 27:
                break
        
        cap.release()
        try:
            cv2.destroyWindow(self.camera_name)
        except:
            pass

# ============================================================
# FLASK ROUTES - VIDEO STREAM
# ============================================================

def generate_frames(camera_id):
    """Video stream generator"""
    global CAMERA_FRAMES, FRAME_LOCK
    
    while True:
        with FRAME_LOCK:
            if camera_id in CAMERA_FRAMES:
                frame = CAMERA_FRAMES[camera_id]
            else:
                # Placeholder frame
                frame = np.zeros((480, 640, 3), dtype=np.uint8)
                cv2.putText(frame, f"Kamera {camera_id} Baslatiliyor...", 
                           (50, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        
        # JPEG encode
        ret, buffer = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 85])
        if not ret:
            continue
        
        frame_bytes = buffer.tobytes()
        
        # MJPEG stream
        yield (b'--frame\r\n'
               b'Content-Type: image/jpeg\r\n\r\n' + frame_bytes + b'\r\n')
        
        time.sleep(0.033)  # ~30 FPS

@app.route('/video/<int:camera_id>')
def video_feed(camera_id):
    """Video stream endpoint"""
    return Response(generate_frames(camera_id),
                   mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/health')
def health():
    """Health check"""
    return jsonify({"status": "ok", "cameras": list(CAMERA_FRAMES.keys())})

def sync_parking_spaces_to_backend(cameras, api_url):
    """
    Tüm kameraların park yerlerini backend'e sync et
    
    Args:
        cameras: Kamera konfigürasyonları listesi
        api_url: Backend API URL'i
    """
    print("\n" + "="*60)
    print("🔄 BACKEND'E PARK YERLERİ SYNC EDİLİYOR...")
    print("="*60)
    
    for camera_config in cameras:
        camera_id = camera_config['id']
        camera_name = camera_config['name']
        coords_file = camera_config['coords']
        
        try:
            # JSON dosyasını oku
            with open(coords_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            
            # Payload hazırla
            payload = {
                'cameraId': camera_id,
                'cameraName': camera_name,
                'parkingSpaces': []
            }
            
            # Park yerlerini ekle
            for space in data.get('spaces', []):
                payload['parkingSpaces'].append({
                    'spaceId': space['space_id'],
                    'coordinates': space['coordinates']
                })
            
            # Backend'e gönder
            response = requests.post(
                f"{api_url}/api/ParkingSpace/sync",
                json=payload,
                timeout=5
            )
            
            if response.status_code == 200:
                result = response.json()
                print(f"✅ {camera_name}: {result['syncedCount']} park yeri sync edildi")
            else:
                print(f"❌ {camera_name}: Sync hatası - Status {response.status_code}")
                print(f"   Response: {response.text[:200]}")
        
        except FileNotFoundError:
            print(f"❌ {camera_name}: Dosya bulunamadı - {coords_file}")
        except requests.exceptions.ConnectionError:
            print(f"❌ {camera_name}: Backend'e bağlanılamadı! ({api_url})")
            print(f"   Backend çalışıyor mu kontrol et!")
        except requests.exceptions.Timeout:
            print(f"❌ {camera_name}: Backend zaman aşımı (timeout)")
        except Exception as e:
            print(f"❌ {camera_name}: Beklenmeyen hata - {str(e)}")
    
    print("="*60)
    print("✅ SYNC İŞLEMİ TAMAMLANDI!")
    print("="*60 + "\n")

# ============================================================
# MAIN
# ============================================================

if __name__ == "__main__":
    TESSERACT_PATH = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
    API_URL = "http://localhost:5197"
    
    # DEBUG MODU
    DEBUG = False
    
    cameras = [
        {"id": 0, "name": "Kamera 1 (A-B Bölümü)", "coords": "camera_0_coordinates.json"},
        {"id": 1, "name": "Kamera 2 (C Bölümü)", "coords": "camera_1_coordinates.json"}
    ]
    
    detectors = []
    threads = []
    
    print("="*60)
    print("🚀 SMART PARKING - FLASK STREAM VERSION")
    print("="*60)
    print("📹 Flask Video Stream: http://localhost:5000")
    print("🎥 Kamera 0: http://localhost:5000/video/0")
    print("🎥 Kamera 1: http://localhost:5000/video/1")
    print("✨ Yol İhlali Cezası: 500 TL (60s)")
    print("✨ Yamuk Park Cezası: 300 TL (60s)")
    print("✨✨ GLOBAL PENALTY SİSTEMİ")
    print("   → Aynı plakaya 1 KEZ ceza!")
    print("   → Cooldown: 5 dakika (300s)")
    print("   → Rollback: API hatası durumunda otomatik")
    print("="*60)
    
    # Backend'e park yerlerini sync et
    sync_parking_spaces_to_backend(cameras, API_URL)
    
    # Kamera thread'lerini başlat
    for cam in cameras:
        detector = CameraDetector(
            camera_id=cam['id'],
            camera_name=cam['name'],
            coords_path=cam['coords'],
            api_url=API_URL,
            tesseract_path=TESSERACT_PATH,
            debug=DEBUG
        )
        detectors.append(detector)
        
        thread = threading.Thread(target=detector.run)
        thread.daemon = True  # Flask kapanınca thread'ler de kapansın
        thread.start()
        threads.append(thread)
    
    print("✅ Kameralar başlatıldı!")
    print("🌐 Flask server başlatılıyor...")
    print("="*60)
    
    # Flask server'ı başlat
    try:
        app.run(host='0.0.0.0', port=5000, threaded=True, debug=False)
    except KeyboardInterrupt:
        print("\n⚠️  Sistem durduruluyor...")
        for detector in detectors:
            detector.running = False
    
    cv2.destroyAllWindows()
    print("✅ Sistem kapatıldı")